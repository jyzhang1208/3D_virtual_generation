...
def save_rgb(base_rgb_list, task_name, episode_idx):
    ...
    if episode_idx == 0:
        ...
        save_path = SAVE_PATH + f"/{task_name[0]}_rgb.pkl"
    ...
    for i in range(100):
        ...
        if i == episode_idx:
            save_dict = {
                ...
                "timesteps": data[i]["timesteps"],
                "imgs": img_embed_ls,
                "lang_emb_77": data[i]["lang_emb_77"],
                "lang_emb": data[i]["lang_emb"],
                "proprio": data[i]["proprio"],
                "lang_goal": data[i]["lang_goal"],
                "actions": data[i]["actions"],
                "poses": data[i]["poses"],
                "terminals": data[i]["terminals"],
